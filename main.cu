...
int main(int argc,char *argv[]) {
    ...
    // int num_objects;
    // hittable** objects;
    // cudaMallocManaged((void **)&objects, sizeof(hittable*));
    // material_list** materials; // pointer for deallocation
    // cudaMalloc((void **)&materials, sizeof(material_list*));

    // create_objects<<<1,1>>>(objects, materials, num_objects);
    // cudaCheckErrors("create objects kernel launch failed");
    // cudaDeviceSynchronize();
    // cudaCheckErrors("post-world-creation synchronization failed");

    // BVH bvh(objects, num_objects);
